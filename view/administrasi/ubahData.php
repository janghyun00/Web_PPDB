		      ...
		      <div class="modal-body">
		        ...
		        <!-- Peserta -->
			      ...
			      <select class="form-control" name="peserta" required="">
			        ...
			        <?php 
			        	$data = mysqli_query($conn, "SELECT Id_Identitas_Siswa, NIK, Nama_Peserta_Didik FROM identitas_siswa WHERE Id_Identitas_Siswa = $row[id_identitas_siswa]");
			        	foreach ($data as $r) {
			        ?>
			        <option value="<?= $r['Id_Identitas_Siswa']; ?>" <?php if ($row['id_identitas_siswa'] == $r['Id_Identitas_Siswa']) { echo 'selected'; } ?>><?= $r['NIK'] . " | " . $r['Nama_Peserta_Didik']; ?></option>
			    	<?php } ?>
			    	<!-- Semua -->
			    	<?php 
			        	$data = mysqli_query($conn, "SELECT Id_Identitas_Siswa, NIK, Nama_Peserta_Didik FROM identitas_siswa WHERE status_administrasi = 0");
			        	foreach ($data as $r) {
			        ?>
			        <option value="<?= $r['Id_Identitas_Siswa']; ?>" <?php if ($row['id_identitas_siswa'] == $r['Id_Identitas_Siswa']) { echo 'selected'; } ?>><?= $r['NIK'] . " | " . $r['Nama_Peserta_Didik']; ?></option>
			    	<?php } ?>
