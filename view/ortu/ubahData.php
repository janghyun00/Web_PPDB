            ...
            <form class="needs-validation" novalidate="" action="../../controller/admin/ortu.php" method="POST">
		      <div class="modal-body">
			      ...
			      <select class="form-control" name="peserta" required="">
			        ...
			        <?php 
			        	include('../../config/connection.php');
			        	$data = mysqli_query($conn, "SELECT Id_Identitas_Siswa, NIK, Nama_Peserta_Didik FROM identitas_siswa WHERE status_ortu = 0");
			        	foreach ($data as $r) {
			        ?>
			        <option value="<?= $r['Id_Identitas_Siswa']; ?>" <?php if ($row['id_identitas'] == $r['Id_Identitas_Siswa']) { echo 'selected'; }?> ><?= $r['NIK'] . " | " . $r['Nama_Peserta_Didik']; ?></option>
			    	<?php } ?>
			    	<!-- identitas diri -->
			    	<?php 
			        	include('../../config/connection.php');
			        	$data = mysqli_query($conn, "SELECT Id_Identitas_Siswa, NIK, Nama_Peserta_Didik FROM identitas_siswa WHERE Id_Identitas_Siswa = '$row[id_identitas]'");
			        	foreach ($data as $s) {
			        ?>
			        <option value="<?= $s['Id_Identitas_Siswa']; ?>" <?php echo 'selected'; ?> ><?= $s['NIK'] . " | " . $s['Nama_Peserta_Didik']; ?></option>
			    	<?php } ?>
			      </select>
			    </div>
